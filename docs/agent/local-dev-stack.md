...
| minio | `bitnamilegacy/minio` (Bitnami's frozen archive — upstream MinIO withdrew all public images, BRAWUKA-711) | access key `coffeemode` / secret `coffeemode123`, S3 API on `localhost:9000`, console on `localhost:9001` |
| minio-init | same `bitnamilegacy/minio` image (bundles the `mc` client) | creates bucket `coffeemode` + anonymous read (mirrors a public R2 bucket) |
