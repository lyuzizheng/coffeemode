...
## Round 17 — Cafe deletion & community handoff — DG146–DG147 (BRAWUKA-10 / BRAWUKA-29)

Owner direction on BRAWUKA-10 (2026-09-02) / BRAWUKA-29: cafe rows are never user-deletable; deletion is checkin-scoped; revive foreclosed.

| # | Decision | Answer |
|---|----------|--------|
| DG146 | Checkin-scoped cafe delete & service-account handoff (replaces revive) | Cafe deletion is checkin-scoped and never deletes the cafe row. `DELETE /api/cafes/[id]` (creator-only, unchanged guards) soft-deletes the caller's live checkins inside a `FOR UPDATE` transaction. If OTHER users have live checkins (>= 1) the cafe is community data: without `{confirm:true}` → 403 `{ code: "cafe_has_other_checkins", n }` (n = others' live count, no mutation); with confirm → caller's checkins soft-deleted AND `created_by` handed to the service account (`SERVICE_ACCOUNT_ID` env var, fallback to fixed UUID `00000000-0000-4000-a000-000000000001` seeded in migration 0016; DG107 override). If no other users' checkins → the cafe remains as a public empty shell (n=0): still listed/searchable with an empty state, excluded from sitemap, `noindex` while empty. Deletion is irreversible; there is no revive entry point. Grandfathered tombstones remain 404; null-`created_by` rows are immutable. |
| DG147 | Cafe visibility as reversible hide (read-filter only) | `cafes.visibility` is `'public' | 'private'` (default `'public'`, `text + CHECK` — not a PG enum), a reversible owner-only toggle via `PATCH /api/cafes/[id]/visibility`. Private is a read-path filter ONLY: excluded from public list/search/nearby/sitemap, 404 for non-owners on `GET /api/cafes/[id]` and the SSR cafe page; the owner always sees their own cafes. `work_stats` and all aggregates keep computing — no freeze, no invalidation. hide ≠ delete: separate routes, never mixed into `deleted_at`. `created_by IS NULL` rows cannot be toggled via API. Read-time display fallback: null `created_by` renders as the service account ("由 CoffeeMode 维护"); query semantics stay strict-equality (no `coalesce` in WHERE). |
...
- 2026-09-02: DG146–DG147 (BRAWUKA-10 / BRAWUKA-29) — Cafe deletion is checkin-scoped and never deletes the cafe row (DG146): `DELETE /api/cafes/[id]` (creator-only) soft-deletes caller checkins; community cafe requires `{confirm:true}` and hands `created_by` to service account (`SERVICE_ACCOUNT_ID` env with fixed UUID fallback); empty shell (n=0) remains public with `noindex` and sitemap exclusion; revive entry point foreclosed. Companion visibility decision (DG147): `cafes.visibility` text + CHECK `public|private` as read-filter only, creator-only toggle, `work_stats` not frozen
