...
| DG147 | Cafe visibility as reversible hide (read-filter only) | Visibility is a reversible hide, separate from deletion: `cafes.visibility text default 'public' check (visibility in ('public','private'))` (not PG enum). `PATCH /api/cafes/[id]/visibility` toggles it (creator-only, `cafes-write`, `requireSameOrigin`). Read filter: `visibility = 'public' and deleted_at is null` for public lists, search, nearby, and sitemap; private cafes are creator-visible only (non-creator `GET [id]` → 404). `work_stats` are not frozen while private. |
