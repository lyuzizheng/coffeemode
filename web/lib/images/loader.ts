...
import { R2_ALLOWED_PUBLIC_HOSTS, r2PublicUrl } from "./constants";
...
export function isR2Image(src: string): boolean {
  ...
  for (const host of R2_ALLOWED_PUBLIC_HOSTS) {
    if (src.startsWith(`https://${host}/`)) return true;
  }
  if (src.startsWith("https://")) {
    try {
      return (R2_ALLOWED_PUBLIC_HOSTS as readonly string[]).includes(new URL(src).hostname);
    } catch {
