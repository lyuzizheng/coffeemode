...
| Rate Limiter Backend | `memory` (single container, ADR-0006) | `memory` (single container, ADR-0006; a multi-replica deploy needs a new shared-store decision — the Postgres backend was deleted by BRAWUKA-378) |
...
- **Deployment Reconfigurability**:
  ...
  - *Current status (2026-09-21, BRAWUKA-598)*: the schedule is **disabled**. It is bound to `coffeemode-web-prod`, which has never been deployed (BRAWUKA-500, owner-deferred), so Dokploy aborts every run at container lookup — `Container not found for application 'app-bypass-solid-state-pixel-pyvr1z'` — before the command executes. Re-enable it as part of BRAWUKA-500 once the prod container is running; the command above is byte-identical to the live schedule. The prod app env also still lacks `DATABASE_URL` and `MULTICA_AUTOPILOT_WEBHOOK_URL`, both of which the job needs.
