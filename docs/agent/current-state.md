...
- Issue #158 adds the safe orphan-original cleanup: `image-service/scripts/clean-orphan-originals.mjs` (npm run clean:orphan-originals) deletes `original/` objects older than RETENTION_DAYS that lack completion metadata OR are still in the "provision" stage (uploaded but never attached) AND are absent from the `web/scripts/export-live-image-keys.mjs` DB export (BRAWUKA-400 reference-aware: referenced stale-marker keys report `would-keep reason:"referenced"`, never deleted; BRAWUKA-725 extends the deletion to final-marked originals: a `checkin`/`cafe` marker proves attachment, not liveness, so an original absent from a COMPLETE export deletes with its `card`/`thumbnail` variants, a live-referenced one is kept, and a missing/empty/malformed export holds every final-marked original back (`finalHeld` in the JSON output — `ALLOW_EMPTY_LIVE_KEYS=1` does not extend to them); BRAWUKA-757 pins the export artifact contract — strict `original/<uuid>.webp` lines closed by a `# live-keys v1 total=<N>` trailer, anything else refuses the run before any listing). complete() now REQUIRES stage metadata: the creation flow sends provision + imageUuid (issue #86 pre-target processing); the post-commit attach leg (BRAWUKA-400) re-marks live originals to `checkin` via a metadata-preserving re-PUT, never blocking the committed creation. The delete paths (BRAWUKA-433) cover the other half: softDeleteCheckIn / deleteCafe / deleteAccount call `deleteUnreferencedPhotos` post-commit, which deletes `original/`+`card/`+`thumbnail/` for ids no LIVE row references (live-only gate `selectLivePhotoReferences` — tombstoned check-ins no longer protect their photos). DRY_RUN=1 default, cursor-paginated, batch-bounded, idempotent, structured JSON output; covered by the images integration suite. Production schedule (export + sweep)/least-privilege creds remain owner actions (#147, #154). Staged browser uploads (BRAWUKA-730) are a second scanned prefix: `staging/` objects delete on age alone (`stagingCandidates`/`stagingDeleted` in the JSON output) — no completion marker, no HEAD, no live-keys export can reference them.
