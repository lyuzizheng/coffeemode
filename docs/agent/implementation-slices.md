...
| discovery-sheet | Bottom sheet + swipe cards + URL sync (core) | READY | 0001, 0002 | design-tokens | none | typecheck, build, e2e | Fixture/API-backed PEEK/HALF/FULL sheet with horizontal cards and back-button-safe URL sync; MapKit binding is a separate slice |
| search-filters | Hybrid search + nomad filters | READY | 0001, 0002 | design-tokens, poi-cache-service | none | typecheck, unit, build, e2e | List/search surface over own cafes and saved POIs; map overlay integration belongs to map-discovery-integration |
| map-discovery-integration | Map-bound discovery and search integration | BLOCKED | 0001, 0002 | map-home, discovery-sheet, search-filters | Apple Developer Program account (MapKit JS token) | typecheck, build, e2e | Bind sheet/card selection, URL sync, map markers, and map search overlay after MapKit is available |
...
| cafe-creation | Creation flow = first check-in (link/manual core) | READY | 0001, 0002 | auth-foundation, places-proxy, poi-cache-service, image-pipeline | none | typecheck, unit, build, e2e | Google/Apple link import + typed name/address manual fallback, dedupe, creator check-in; MapKit map-tap/search is the separate map-creation-entry slice; populate `cafes.tz` from `location` (IANA lookup) so open-now works (issue #77) |
| map-creation-entry | Map-tap and MapKit search creation entry | BLOCKED | 0001, 0002 | map-home, cafe-creation | Apple Developer Program account (MapKit JS token) | typecheck, build, e2e | MapKit search, map-tap creation, reverse geocoding, map FAB binding, and map-bound creation entry points |
| checkin-system | Check-in drawer + sliders | READY | 0001, 0002 | design-tokens, image-pipeline | none | typecheck, unit, build, e2e | 0-100 sliders, policy chips with unknown, photos, repeat check-in flow; can use cafe/check-in APIs or fixtures before MapKit |
...
| navigation-prompt | Navigation tracking + return prompt | READY | 0001 | design-tokens | none | typecheck, unit, e2e | Navigation events recorded; ClassPass-style check-in prompt on return, using APIs or fixtures independently of map UI |
| profile-page | User profile page | READY | 0001, 0002 | auth-foundation, design-tokens | none | typecheck, build, e2e | /profile lists the user's check-ins and cafes without a map dependency |
| seo-sharing | SSR deep links + share flow | READY | 0001, 0002 | design-tokens | none | typecheck, build, e2e | /cafes/[id] SSR deep link, OG images, Web Share API without discovery-sheet coupling |
