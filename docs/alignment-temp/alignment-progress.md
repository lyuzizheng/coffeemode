...
## Round 17 — Cafe deletion & community handoff — DG131 (BRAWUKA-10 / BRAWUKA-29)
...
- 2026-09-02: DG131 (BRAWUKA-10 / BRAWUKA-29) — Cafe deletion is checkin-scoped and never deletes the cafe row. `DELETE /api/cafes/[id]` (creator-only) soft-deletes caller checkins; community cafe requires `{confirm:true}` and hands `created_by` to service account (`SERVICE_ACCOUNT_ID` env with fixed UUID fallback); empty shell (n=0) remains public with `noindex` and sitemap exclusion; revive entry point foreclosed
