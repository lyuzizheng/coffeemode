...
| issue-46-sw-api-cache | Service worker must not cache user-specific API GETs | IN-PROGRESS | 0001 | none | none | typecheck, unit, build | Explicit cafes/checkins rule already removed; real hole is serwist defaultCache catch-all `apis` NetworkFirst (24h) catching new /api/cafes routes. Fix: catch-all network-only for /api/ in RUNTIME_RULES |
