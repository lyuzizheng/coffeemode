...
| API2 | Implement `GET /api/cafes`, `GET|DELETE /api/cafes/[id]`, `POST /api/cafes/[id]/revive` (DELETE = creator-only soft delete retaining location tombstone: 401 unauthenticated, 404 unknown/tombstoned, 403 non-creator, `cafes-write` rate limit; revive = creator-only tombstone restoration: 401 unauth, 400 bad UUID, uniform 404 for unknown/live/non-creator/null-`created_by` tombs without tombstone oracle, 409 POI conflict with `{ error: "conflict", replacement_id }` when external POI is live on a new row, 200 `{ ok: true, id }`, `requireSameOrigin` + `cafes-write` rate limit; `GET /api/cafes/[id]/recovery` reports `replacement_id` when tomb's POI is live; `created_by IS NULL` rows are not API-deletable/revivable, moderation is post-MVP — DG125) | backend | `web/app/api/cafes/[id]/route.ts`, `web/app/api/cafes/[id]/revive/route.ts` |
