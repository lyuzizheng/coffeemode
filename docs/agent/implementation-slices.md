...
| issue-119-r2-head-error | Preserve image-service storage errors instead of mapping them to not_found | COMPLETE | 0001, 0003 | issue-118-integration-suite-safety | none | typecheck, unit, preflight | `headObject` returns null only for 404; `R2HeadObjectError` preserves non-2xx status; `redirect: "manual"` and validated `Content-Length`; tests cover the `R2_ENDPOINT` path, native `R2_BUCKET` binding, and the route's sanitized 500 envelope (#119); real MinIO/R2 round-trip remains a separate follow-up |
