...
**DG151 — The user-location blue dot is a first-class map layer.** `IMapProvider.setUserLocation(location | null)` renders the granted fix as a pulsing blue dot (accuracy circle + core dot, separate source/layers from cafe pins — never folded into `CafeSummary`). Session-persistent per DG120: the dot restores from `profiles.last_location` (signed-in) or the onboarding localStorage copy (anonymous) on launch; a locate-button re-tap recenters on it. The first user camera gesture latches `onCameraGesture` (DG119) so a late grant never steals the viewport.
