...
### Cafe lifecycle — delete guard, orphan shell & visibility (DG146 / DG147, owner 2026-09-02, replaces revive)

**DG146 — Delete is checkin-level, not cafe-level (irreversible).** No `revive`/`POST /revive`; old `reviveCafe` + tests removed, `cafes.deleted_at` kept only for legacy tombstones (404+recovery unchanged, no migration). Guard: `otherLiveCheckins = count(*) where cafe_id=$1 and user_id!=$caller and deleted_at is null` (no `cafes.is_deleted` — partial index on `checkins.deleted_at IS NULL` suffices; `is_deleted boolean` would duplicate it). `otherLiveCheckins>=1` → `DELETE` without `{confirm:true}` returns `403 {code: cafe_has_other_checkins, n}` with zero side effects (`FOR UPDATE` only on confirm path via `withTransaction`/`postgres.ts:115`). With confirm the transaction soft-deletes caller's live checkin (`deleted_at=now()`, `work_stats` recompute, gallery `source` filter) and if `cafes.created_by==caller` migrates it to `SERVICE_ACCOUNT_ID` (`00000000-0000-4000-a000-000000000001`, env `SERVICE_ACCOUNT_ID`, `profiles` seeded `INSERT ... ON CONFLICT DO NOTHING`; `profiles.id` has no FK to `auth.users` per `0001_init.sql:8`, later Supabase service account + admin site will own it). When `otherLiveCheckins==0` the same transaction leaves an orphan shell cafe with `n=0` — publicly visible (Q2 A) but excluded from `sitemap.xml` and detail sends `noindex` while thin. Same-POI re-create remains `409` because the shell occupies `idx_cafes_gplace`/`idx_cafes_apple_poi_id where deleted_at is null`. `created_by IS NULL` rows fallback to service account at read time (`toPublicCafeDetail`) (Q4).

**DG147 — Visibility is reversible hide, separate from delete.** `cafes.visibility text default 'public' check (visibility in ('public','private'))` (not PG enum). `PATCH /api/cafes/[id]/visibility` toggles it, owner only, `cafes-write` + `requireSameOrigin`. Read filter: `visibility='public' and deleted_at is null` for public lists/search/nearby/sitemap; `private` is owner-visible only (non-owner `GET [id]` → 404). `work_stats` not frozen while private. `SERVICE_ACCOUNT_ID` is env-owned (owner 06-18 overrides DG107 typed-config advice for this id).

### Helpful ranking — daily time-decayed snapshot (DG148, owner 2026-09-13, V2 #140)

**DG148 — Helpful ordering moves from the live `likes_count` tuple to a versioned daily snapshot.** Score: `score = likes_count × 0.5^(age_days / halfLifeDays)`, `halfLifeDays = 14`, age measured from `checkins.visited_at`. Zero-like check-ins score 0 and order among themselves by `visited_at DESC, id DESC` (degrades into Newest ordering). One global run per job execution — `helpful_ranking_runs(status building|active|superseded)` with a partial unique index enforcing at most one `active` run, plus `helpful_ranking_entries(run_id, cafe_id, checkin_id, score, likes_count, visited_at)` with frozen inputs. The nightly job (`web/scripts/snapshot-helpful-ranking.mjs`, second step of `nightly-recompute.yml` at 02:00 UTC) builds a `building` run, fills entries in one pass, then flips it `active` (superseding the prior run); reads join the active run's entries.

### Map chrome & settings (DG149–DG152, owner 2026-09-19, BRAWUKA-504)

**DG149 — Account deletion is a real product surface, not a stub.** `DELETE /api/profile` (bucket `profile-write`, `requireSameOrigin`) runs one `FOR UPDATE` transaction: soft-delete the caller's live checkins (`deleted_at=now()`, `work_stats` recompute per affected cafe, gallery filter), move `cafes.created_by` to `SERVICE_ACCOUNT_ID` (same orphan-shell rule as DG146), delete `checkin_likes`/`navigations`/`image_upload_intents` rows, detach tombstones (`checkins.user_id = null` — the FK has no ON DELETE clause and would otherwise roll the tx back), then `profiles` row delete + Supabase admin `auth.admin.deleteUser`. `GET /api/profile/export` (bucket `profile-export`, 5/min) returns the full account as one JSON document — profile, checkins (soft-deleted included), created cafes, navigations, checkin_likes — `Content-Disposition: attachment`, `Cache-Control: no-store`. Both live under `/settings` → Account; delete sits behind a `DangerConfirm` dialog.

**DG150 — The map chrome is a two-cluster system.** Bottom-right (mobile + desktop): the add-cafe FAB with the locate button stacked directly above it — the locate button leaves the sheet-peek slot and the desktop top-right corner it shared with the account chip. Top-right: the account/menu cluster (`MapAccountChip`) — avatar-initial → `/profile` when signed in, "Sign in" → `/profile` when not, plus the droplet `AppMenu`. `gateMapOverlay` still hides the whole cluster above PEEK.

**DG151 — The user-location dot is a first-class map layer.** `IMapProvider.setUserLocation(location | null)` renders the granted fix as the brand-accent dot (accuracy halo + core disc in `--accent` terracotta, separate source/layers from cafe pins — never folded into `CafeSummary`). Static, not pulsing: the pulse affordance lives on the locate glyph (one replay per tap / `?locate=1` hint), and a rAF-driven map-layer pulse costs battery for zero information gain. Session-persistent per DG120: the dot restores from `profiles.last_location` (signed-in) or the onboarding localStorage copy (anonymous) on launch; a locate-button re-tap recenters on it. The first user camera gesture latches `onCameraGesture` (DG119) so a late grant never steals the viewport.

**DG152 — `/settings` is the single home for preferences and account actions.** Grouped rows: Preferences (theme light/dark/system, theme variant, language en/zh, ranking), Account (profile link, export data, sign out, delete account), Legal (`/privacy`, `/terms`, `/about` — static bilingual pages sharing one `LegalPage` shell). Anonymous visitors keep Preferences + Legal; the Account group renders a sign-in gate instead of hiding the page. The droplet `AppMenu` (map + page variants) is the global entry: theme, language, settings, sign-in/out — it replaces every page-local ThemeToggle/SignOutButton pair.

**DG153 — Zero-data profile renders a starter card, not four empty tabs.** When a signed-in user's check-ins query returns an empty first page, `/profile` shows a three-step guide (locate → add a cafe → check in) deep-linking to `/?locate=1` and `/?create=1`. Skipping writes a localStorage dismissal (`profile-guide-store`) — the card never returns once dismissed or once data exists (DG39, no nag surfaces). `?locate=1` primes the locate button's pulse only — DG112 still requires a tap, never an auto-prompt.
...
34. **MVP hosting stays self-hosted VPS + Cloudflare Workers; managed Postgres is deferred.** The Supabase Pro vs Neon comparison (2026-08-27) showed a ≤$20/mo difference at MVP scale — not worth split-brain operations while Supabase Auth is already required. Hyperdrive is bundled into Workers plans, not a separate add-on. Move triggers are listed under Post-MVP. Rate limiting enforces in memory on the single container **(amended by 34a, backend deleted by BRAWUKA-378)**.
34a. **Owner decision (2026-08-28), amends 34: Supabase hosts the main Postgres (free tier first); Cloudflare Workers run the microservices; the single app container runs rate limiting in-memory.** One platform for auth + data removes split-brain operations, and the free tier covers MVP scale (current seed is 14 cafes / 28KB — far below the 500MB cliff). BRAWUKA-378 (owner 2026-09-18) deletes the Postgres rate-limit backend outright — a future multi-instance deploy needs a new shared-store decision. Known free-tier cliffs and mitigations: 7-day inactivity pause — the nightly recompute workflow doubles as keep-alive once `DATABASE_URL` is a GitHub secret; 500MB then…

34b. **BRAWUKA-637 acceptance (2026-09-22): single-instance in-memory is the standing constraint** — see ADR-0006. A multi-replica deploy is gated on a new shared-store decision (leading candidate Redis/Upstash); until then both stacks run one web container and per-process buckets are exact.

### Launch gate, seeding & sparse data (owner 2026-09-07, BRAWUKA-109 thread)

35. **Launch gate = every external dependency integrated and the production loop verified bug-free; release form = silent.** The gate is: the basemap tile host (OpenFreeMap public instance — self-hosting permanently off the table per BRAWUKA-321 owner decision; the Apple Developer/MapKit requirement was eliminated by BRAWUKA-308/311), Supabase production DB + auth, Google Places + the POI worker, and domain/VPS/Cloudflare all wired up, with the full production flow working without bugs. Once that holds, the product may ship immediately — no announcement, no marketing, no growth actions; the owner and friends use it first. A seed-density threshold is explicitly NOT part of the gate (supersedes the PM draft in BRAWUKA-109 question 4, which had proposed one).
...
| API2 | Implement `GET /api/cafes`, `GET|DELETE|PATCH /api/cafes/[id]` + `PATCH /api/cafes/[id]/visibility` — **DG146/DG147 (owner 2026-09-02, revive veto)**: `DELETE` is checkin-level with guard otherLiveCheckins>=1 → without `{confirm:true}` 403 `{code: cafe_has_other_checkins,n}` zero changes; with confirm soft-deletes all of the caller's live checkins on that cafe (+ moves `created_by` to `SERVICE_ACCOUNT_ID` env `00000000-0000-4000-a000-000000000001` if caller was owner, in `FOR UPDATE` transaction), `work_stats` recompute, gallery filter; when only caller's checkin remains deletion leaves orphan shell (`n=0`) publicly visible but sitemap excluded + detail robots meta noindex (via Next `metadata.robots`) (Q2 A, no ranking demotion, thin content). Same POI re-create still 409 (shell occupies partial unique indexes). Old `cafes.deleted_at` tombstones stay 404+recovery, no migration. No `is_deleted` boolean — guard uses `checkins.deleted_at IS NULL` partial indexes (existing `idx_checkins_* where deleted_at is null`); `is_deleted` would be redundant. `PATCH /visibility` toggles `public|private` (`text+CHECK`, owner only, `cafes-write`, `requireSameOrigin`, read-filter only, `work_stats` not frozen). Legacy `reviveCafe` removed in S1. (Tracks #229) | backend | `web/app/api/cafes/[id]/route.ts`, `web/app/api/cafes/[id]/visibility/route.ts` |
...
| API5 | Implement `POST /api/navigations` and pending-prompt endpoint *(implemented in #149: record + `GET /api/navigations/prompt` + `POST /api/navigations/[id]/resolve`)* | backend | `web/app/api/navigations/*` |
...
- Supabase Pro upgrade ($25/mo) for the main database (decision 34a). Triggers: the 7-day pause bites real usage, the 500MB read-only cliff or 5GB egress approaches, backup/PITR becomes a requirement, or a multi-instance deploy lands (which also needs a new shared rate-limit store decision — the Postgres backend was deleted by BRAWUKA-378, see ADR-0006). Hyperdrive edge pooling (bundled in Workers plans) only if edge read latency becomes a measured problem.
