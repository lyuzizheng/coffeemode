...
| work-profile | Aggregation + dual scores | COMPLETE | 0001 | design-tokens | none | typecheck, unit, integration, build | Incremental work_stats, experience + weighted scores, nightly recompute; independent of map UI (#146) |
| map-home | Apple MapKit home map | BLOCKED | 0001, 0002 | scaffold-nextjs, design-tokens | Apple Developer Program account (MapKit JS token, #131); Kimi K3 map-home design artifact | typecheck, build, e2e, visual | Full-screen map, custom markers, clustering, dark scheme, geolocation (#132) |
...
| discovery-sheet | Responsive discovery core + swipe cards + URL sync | BLOCKED | 0001, 0002 | design-tokens, work-profile | Kimi K3 discovery design artifact | typecheck, unit, integration, build, e2e, visual | Mobile PEEK/HALF/FULL and desktop sidebar/detail column share selection + one-push/then-replace URL state; real public-safe CafeSummary/detail/check-in DTOs, Helpful/Newest cursor feeds, accepted recovery/accessibility/breakpoint behavior, no MapKit or SSR-route ownership (#133) |
| search-filters | Hybrid search + nomad filters | BLOCKED | 0001, 0002 | design-tokens, poi-cache-service | Kimi K3 search design artifact | typecheck, unit, build, e2e, visual | Map-independent list/search surface over own cafes, saved POIs, and optional live results; map overlay integration belongs to map-discovery-integration (#135) |
| map-discovery-integration | Map-bound discovery and search integration | BLOCKED | 0001, 0002 | map-home, discovery-sheet, search-filters | Apple Developer Program account (MapKit JS token, #131); Kimi K3 integration design artifact | typecheck, build, e2e, visual | Bind sheet/card selection, URL sync, map markers, and map search overlay after MapKit is available (#134) |
...
| cafe-creation | Creation flow = first check-in (link/provider core) | BLOCKED | 0001, 0002, 0004 | auth-foundation, places-proxy, poi-cache-service, image-pipeline | Kimi K3 review of the PR #128 creation UI | typecheck, unit, build, visual | Runtime implementation is in PR #128: Google/Apple link import + provider search, POI persistence, dedupe, and creator check-in; visual acceptance and merge wait for Kimi review; map-pin/manual creation is deferred (#130) |
| map-creation-entry | Map-tap and MapKit search creation entry | BLOCKED | 0001, 0002 | map-home, cafe-creation | Apple Developer Program account (MapKit JS token, #131); Kimi K3 map-creation design artifact | typecheck, build, e2e, visual | MapKit search, map-tap creation, reverse geocoding, map FAB binding, and map-bound creation entry points (#136) |
| checkin-system | Check-in drawer + sliders | BLOCKED | 0001, 0002 | design-tokens, image-pipeline | Kimi K3 check-in design artifact | typecheck, unit, build, e2e, visual | 0-100 sliders, policy chips with unknown, photos, repeat check-in flow; can use cafe/check-in APIs or fixtures before MapKit |
| navigation-prompt | Navigation tracking + return prompt | BLOCKED | 0001, 0002 | design-tokens | Kimi K3 navigation-prompt design artifact | typecheck, unit, e2e, visual | Navigation events recorded; ClassPass-style check-in prompt on return, using APIs or fixtures independently of map UI |
| profile-page | User profile page | BLOCKED | 0001, 0002 | auth-foundation, design-tokens | Kimi K3 profile design artifact | typecheck, build, e2e, visual | /profile lists the user's check-ins and cafes without a map dependency |
| seo-sharing | SSR deep links + share flow | BLOCKED | 0001, 0002 | design-tokens | Kimi K3 cafe-detail/share design artifact | typecheck, build, e2e, visual | /cafes/[id] SSR deep link, OG images, Web Share API without discovery-sheet coupling |
| deploy-vps | Docker + VPS + CDN + CI/CD | BLOCKED | 0001, 0003 | work-profile, search-filters, navigation-prompt, seo-sharing | Domain + VPS + Cloudflare account (pending-user-actions §7) | build, deploy | Production on VPS behind Cloudflare CDN with green pipeline |
| cleanup-legacy | Remove old Vite frontend + Java backend | BLOCKED | 0001 | deploy-vps | deploy-vps not complete | build, e2e | Legacy code removed after feature parity is verified |
...
| auth-migration-stats | Auth middleware, schema migration, check-in types, work_stats aggregation | COMPLETE | 0001, 0004 | auth-foundation | none | typecheck, unit, build | `web/proxy.ts` refreshes only when a Supabase session cookie is present, using `getSession()`; route handlers call `getUser()` for auth before any Postgres write; `0002_checkins_and_indexes.sql` adds soft-delete, likes, indexes; `aggregate.ts` computes work_stats incrementally |
...
| issue-76-visual-gate | Playwright rendered-page smoke command | COMPLETE | 0003 | none | none | typecheck, unit, build | `web/scripts/visual-smoke.mjs` + `check:visual` remain local/manual browser evidence; the unbounded PR job was removed by #125 because it had no visual baselines |
| issue-77-cafe-timezone | cafes.tz column + tz-correct open-now evaluation | COMPLETE | 0001 | none | none | typecheck, unit, build | Migration 0005 adds `cafes.tz` (IANA); `web/lib/hours.ts` `isOpenAt` evaluates weekly hours in cafe-local time; cafe creation populates it from coordinates at write time |
...
| issue-117-integration-ci-gate | Enforce real-DB integration suite in CI | COMPLETE | 0003 | none | none | integration, preflight | Unified `.github/workflows/ci.yml` starts pinned PostGIS and runs `npm run test:integration` for classified DB-capable changes; plain unit tests remain fast/skipped without Docker (#117, #125) |
...
| issue-119-r2-head-error | Preserve image-service storage errors instead of mapping them to not_found | COMPLETE | 0001, 0003 | issue-118-integration-suite-safety | none | typecheck, unit, preflight | `headObject` returns null only for 404; `R2HeadObjectError` preserves non-2xx status; `redirect: "manual"` and validated `Content-Length`; tests cover the `R2_ENDPOINT` path, native `R2_BUCKET` binding, and the route's sanitized 500 envelope (#119); real MinIO/R2 round-trip covered by the #156 images integration suite |
