...
BRAWUKA-185 (#358) 造成的 +5；`config-schema.ts` 是 BRAWUKA-184 (#357) 引入。基线自记录值起只降不升。
`web/components/discovery/checkin-feed.tsx`（440 → 434/#361，实测 433）已由 BRAWUKA-198 (#368)
拆分为 `use-checkin-feed.ts` + `feed-card.tsx` + 薄壳（177 行）并毕业，按 §7.4 从表与机器基线移除。
