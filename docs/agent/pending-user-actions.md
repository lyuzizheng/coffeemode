...
- [x] ~~Better Stack account + per-environment sources for rate-limit/observability alerts (DG129, BRAWUKA-235)~~ — **retired 2026-09-21**. Sources `coffeemode-rate-limit-staging` / `coffeemode-rate-limit-prod` were created 2026-09-17 and verified live (one synthetic `rate_limited` event per source, each confirmed back through the query API within ~1 min), but the owner's call is a direct cutover with no dual-run (BRAWUKA-605 §6 decision 1), so the app-side POST and the `BETTER_STACK_INGEST_*` pair are deleted and the sources receive nothing. `rate-limit-alert.ts` now emits one structured `logWarn` line per denial, shipped to Grafana Cloud Loki by `otlp-logs.ts` (BRAWUKA-607). No owner action remains here.
...
- [x] ~~(Optional) Decide whether the rate-limit alert sink (`web/lib/observability/rate-limit-alert.ts`, DG129) moves from Better Stack to Grafana Cloud~~ — decided 2026-09-21 (BRAWUKA-605 §6 decision 1): direct cutover, no dual-run. The Better Stack POST is deleted; the line ships to Loki via `otlp-logs.ts` (BRAWUKA-607).
...
- [ ] Verify after the paste: Tempo returns `coffeemode-web` traces and the service map shows edges; `traces_spanmetrics_*` appears in Prometheus with `span_name` as a route template (`GET /api/cafes/[id]`), not a raw path. Note `http_route` is **not** a spanmetrics default label — the default set is `service` / `span_name` / `span_kind` / `status_code`, and the template rides on `span_name`. Adding `http.route` as an extra dimension is optional Grafana-side config, not required for per-route RED.
