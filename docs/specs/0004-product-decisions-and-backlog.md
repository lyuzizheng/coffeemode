...
**DG148 — Helpful ordering moves from the live `likes_count` tuple to a versioned daily snapshot.** Score: `score = likes_count × 0.5^(age_days / halfLifeDays)`, `halfLifeDays = 14`, age measured from `checkins.visited_at`. Zero-like check-ins score 0 and order among themselves by `visited_at DESC, id DESC` (degrades into Newest ordering). One global run per job execution — `helpful_ranking_runs(status building|active|superseded)` with a partial unique index enforcing at most one `active` run, plus `helpful_ranking_entries(run_id, cafe_id, checkin_id, score, likes_count, visited_at)` with frozen inputs. The nightly job (`web/scripts/snapshot-helpful-ranking.mjs`, second step of `nightly-recompute.yml` at 02:00 UTC) builds a `building` run, fills entries in one `INSERT … SELECT` over non-deleted check-ins, then publishes atomically in a single transaction (supersede old active + activate new). A failed or interrupted run leaves the previous version serving — never a partial snapshot. Superseded runs are retained `snapshotRetentionDays = 7` days, then deleted (entries cascade). Read path: Helpful keys on `(score, visited_at, id)` within the active run, still joins `checkins` and filters `deleted_at IS NULL` live (snapshot freezes order, not visibility); with no active run it falls back to the MVP live tuple for zero-downtime rollout. Cursor v2 is opaque and carries `{v:2, mode, run, score, visited_at, id}`; a cursor whose `run` is not the active run returns `410 {code:"cursor_version_expired"}` — the client restarts from page one. Newest mode, 20-item page size, and the malformed-cursor 400 are unchanged. Constants live in `app.yaml` (`feed.helpful.halfLifeDays`, `feed.helpful.snapshotRetentionDays`), never in request parameters.
