...
| work-profile | Aggregation + dual scores | READY | 0001 | design-tokens | none | typecheck, unit, build | Incremental work_stats, experience + weighted scores, nightly recompute; independent of map UI |
...
| discovery-sheet | Responsive discovery core + swipe cards + URL sync | BLOCKED | 0001, 0002 | design-tokens, work-profile | Kimi K3 discovery design artifact; check-in feed ordering/pagination decision; public cafe/check-in DTO and author-identity privacy contract | typecheck, unit, integration, build, e2e, visual | Mobile PEEK/HALF/FULL and desktop sidebar/detail drawer share selection + one-push/then-replace URL state; real CafeSummary/detail/check-in data, no MapKit or SSR-route ownership (#133) |
