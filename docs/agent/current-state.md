...
- Issue #158 adds the safe orphan-original cleanup: `image-service/scripts/clean-orphan-originals.mjs` (npm run clean:orphan-originals) deletes only `original/` objects older than RETENTION_DAYS that LACK completion metadata (`x-amz-meta-targettype` — complete() always sets it on live gallery originals). DRY_RUN=1 default, cursor-paginated, batch-bounded, idempotent, structured JSON output; covered by the images integration suite. Production schedule/least-privilege creds remain owner actions (#147, #154).
