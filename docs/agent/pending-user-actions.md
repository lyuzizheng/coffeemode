...
- [x] ~~Better Stack account + per-environment sources for rate-limit/observability alerts (DG129, BRAWUKA-235)~~ **Retired by BRAWUKA-611 (2026-09-21)** — the four CoffeeMode sources, both dashboards, all four chart alerts and the `coffeemood.com` uptime monitor are deleted, and the two ingest env vars are removed from the Dokploy staging app. The rate-limit event now reaches Grafana Cloud Loki as a structured `logWarn` line (`code: "rate_limited"`) and is alerted on by `CoffeeMode — Rate-limit flood`. **No owner action remains here — do not paste ingest credentials.**
