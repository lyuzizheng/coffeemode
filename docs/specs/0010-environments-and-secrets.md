...
| `OTEL_EXPORTER_OTLP_ENDPOINT` + `OTEL_EXPORTER_OTLP_HEADERS` | Dokploy env per env (Grafana Cloud OTLP gateway; `Authorization: Basic base64(instanceID:token)` in the header) | — (write-only token; never `NEXT_PUBLIC_*`) |
