...
# ------------------------------------------------------------------------------
stage "Step 1/4: Pre-Migration Snapshot Identification"

if [[ -z "$BACKUP_FILE" ]]; then
  log "No explicit snapshot passed. Searching for latest production pre-migration snapshot..."
  SEARCH_DIRS=(
...
log "Rolling back application container to previous release state..."
...
if [ "$DRY_RUN" = false ]; then
  ...
  ok "Web container reverted."
else
  ok "[DRY-RUN] Application container rollback simulated."
fi
